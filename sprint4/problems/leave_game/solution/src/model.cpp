...
void Game::Tick(double dt) {
    for (auto& session : sessions_) {
        auto retired_dogs = session->Tick(dt, dog_retirement_time_);
...
void Game::RetirePlayer(const std::shared_ptr<Dog>& dog) {
    if (!dog) return;

    auto it = std::find_if(players_.begin(), players_.end(), [&dog](const auto& player) {
        return player && player->GetDogPtr() == dog;
    });
    if (it == players_.end()) return;

    auto player = *it;
    RetiredPlayerRecord record{dog->GetName(), dog->GetScore(), dog->GetPlayTime()};

    players_.erase(it);
    tokens_.RemovePlayer(player);

    if (retirement_callback_) {
        retirement_callback_(record);
    }
}
...
void Game::RestoreState(const serialization::SavedState& state) {
    players_.clear();
    ...
    for (const auto& p_repr : state.GetPlayers()) {
        ...
        if (p_repr.GetId() >= next_player_id_) {
            next_player_id_ = p_repr.GetId() + 1;
        }
    }
}
...
std::vector<std::shared_ptr<Dog>> GameSession::Tick(double dt, double retirement_time) {
    std::vector<collision_detector::Gatherer> gatherers;
