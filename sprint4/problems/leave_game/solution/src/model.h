...
public:
    ...
    void UpdatePlayAndIdleTime(double dt) noexcept {
        play_time_ += dt;
    ...
    void SetIdleTime(double idle_time) noexcept { idle_time_ = idle_time; }
    void SetPlayTime(double play_time) noexcept { play_time_ = play_time; }
...
public:
    ...
    std::vector<std::shared_ptr<Dog>> Tick(double dt, double retirement_time);
...
public:
    ...
    void RemovePlayer(const std::shared_ptr<Player>& player) {
        ...
        if (it == player_to_token_.end()) return;
...
public:
    ...
    using RetirementCallback = std::function<void(const RetiredPlayerRecord&)>;
...
private:
    ...
    void RetirePlayer(const std::shared_ptr<Dog>& dog);
