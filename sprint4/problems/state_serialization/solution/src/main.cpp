...
struct Args {
    ...
    std::optional<uint64_t> save_state_period; // мкс
};
...
std::optional<Args> ParseCommandLine(int argc, char* argv[]) {
    ...
    desc.add_options()
        ...
        ("state-file", po::value<std::string>()->value_name("file"s), "set state save file path")
        ("save-state-period", po::value<uint64_t>()->value_name("milliseconds"s), "set auto-save period");
    ...
    try {
        ...
        if (args->state_file) {
            ...
            if (std::filesystem::exists(state_path)) {
                try {
                    game.RestoreState(state_path);
                } catch (const std::exception& ex) {
                    BOOST_LOG_TRIVIAL(error) << "Failed to restore state from file: " << ex.what();
                    return EXIT_FAILURE;
                }
            }

            if (args->save_state_period) {
                game.EnableAutoSave(state_path, std::chrono::milliseconds{*args->save_state_period});
            } else {
                game.SetStateFile(state_path);
            }
        }
        ...
        signals.async_wait([&ioc, &game, state_file = args->state_file](const boost::system::error_code&, int) {
            ...
            if (state_file) {
                try {
                    ...
                    BOOST_LOG_TRIVIAL(error) << "Error saving state on exit: " << e.what();
                }
            }
            ioc.stop();
        });
