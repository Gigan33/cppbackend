...
class Dog {
public:
    explicit Dog(std::string name, uint32_t id) : name_(std::move(name)), id_(id) {}
...
public:
    ...
    std::shared_ptr<Dog> CreateDog(const std::string& dog_name) {
        ...
        auto dog = std::make_shared<Dog>(dog_name, dog_id);
        dogs_.push_back(dog);
        return dog;
    }
private:
...
private:
    ...
    // Изменили на unique_ptr, чтобы класс стал перемещаемым
    std::unique_ptr<std::random_device> random_device_;
    
    // Разыменовываем указатель (*random_device_) при инициализации генераторов
...
public:
    ...
    std::pair<Token, uint32_t> JoinGame(const std::string& map_id, const std::string& user_name) {
        ...
        auto dog = session->CreateDog(user_name);
...
private:
    ...
    // Новые поля управления состоянием
