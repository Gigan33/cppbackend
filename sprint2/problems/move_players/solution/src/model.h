...
public:
    ...
    const PointD& GetPosition() const noexcept { return position_; }
    const SpeedD& GetSpeed() const noexcept { return speed_; }
...
public:
    ...
    std::shared_ptr<Dog> CreateDog(const std::string& dog_name) {
        ...
        // По ТЗ: пёс должен появиться на дороге. Ставим его на первую дорогу карты.
        PointD spawn_pos{0.0, 0.0};
